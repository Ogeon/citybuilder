...
impl<'s> Gui<'s> {
	pub fn new(dimensions: Vector2f, padding: i32, horizontal: bool, style: GuiStyle, entries: Vec<(&str, String)>) -> Gui<'s> {
		...
		Gui {
			...
			entries: entries.move_iter().map(|(text_str, message_str)| {
				let mut text = Text::new_init(text_str, style.font.clone(), (dimensions.y - style.border_size - padding as f32) as uint).unwrap();
	...
	pub fn set_entry_text(&mut self, entry: uint, text: &str) {
		if entry < self.entries.len() {
			self.entries.get_mut(entry).text.set_string(text);
		}
	}
